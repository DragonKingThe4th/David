...
pairs = [
    ...
    [
        r"what is the date today|what day is it|what's the date", # New rule for date
...
def chat_with_llm_and_voice_output():
    ...
    while True:
        ...
        if any(phrase in user_input.lower() for phrase in ["what time is it", "tell me the time", "current time"]):
            now = datetime.now()
            formatted_time = now.strftime("The current time is %I:%M %S %p.") # %I for 12-hour, %M for minute, %p for AM/PM
        ...
        Google_Search_match = nltk.re.search(r"search (.*) on google", user_input.lower())
